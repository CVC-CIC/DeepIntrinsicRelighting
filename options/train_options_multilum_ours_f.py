...
class TrainOptions(BaseOptions):
    def __init__(self, which_experiment):
        ...
        # Setting for dataset
        self.dataset_mode = 'relighting_single_image_multilum'  # name of the dataset
        self.dataset_setting = 'ALL'   # 'IAN' 'SILT' 'ALL'
        # 'SILT' setting: SILT : Self-supervised Lighting Transfer Using Implicit Image Decomposition.
        # For this paper we picked nine illumination conditions, consistent across all scenes of the
        # dataset, and chose one unpaired image setting to act as target lighting domain.
        # 'IAN' setting: Designing an Illumination-Aware Network for Deep Image Relighting.
        # In our experiment, we take dir_0 as input illumination settings and dir_17 as output
        # illumination settings in dataset to train all methods for evaluation.
        if self.dataset_setting == 'ALL':
            self.anno = 'data/multi_illumination/train.txt'  # the anno file from prepare_dataset.py
            self.anno_validation = 'data/multi_illumination/val.txt'
        elif self.dataset_setting == 'SILT':
            self.anno = 'data/multi_illumination/train_setting1.txt'  # the anno file from prepare_dataset.py
            self.anno_validation = 'data/multi_illumination/val_setting1.txt'
        elif self.dataset_setting == 'IAN':
            self.anno = 'data/multi_illumination/train_setting2.txt'  # the anno file from prepare_dataset.py
            self.anno_validation = 'data/multi_illumination/val_setting2.txt'
        else:
            raise Exception("Error: dataset_setting")
        # Setting for GPU
        ...
        self.light_type = "probes"  # ["pan_tilt_color" | "Spherical_harmonic"]
        ...
        self.display_ncols = -1  # if positive, display all images in a single visdom web panel with certain number of images per row.')
